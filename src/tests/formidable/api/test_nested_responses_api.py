...
class NestedResponsesApiTests(APITestCase, URLPatternsTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.list_url = reverse("sections-responses-list", args=[self.section.id])
    ...
    def test_add_responses_successful(self):
        ...
        data = {
            "responses": [
                {"value": "random value with name", "field": self.fields[0].id},
                {"value": "some thing here", "field": self.fields[1].id},
                {"value": "anther value here", "field": self.fields[2].id},
            ]
        }
